...
| 功能 | 原理 | 实测触发情况 |
|------|------|-------------|
| **纹理图集上限** | 限制 `GlDevice.createTexture()` 宽高 ≤ `maxAtlasSize` | ✅ 单次会话 26 次截断（blocks.png 16384→4096） |
...
**MC 1.21.11 + Fabric 0.19.3 + Sodium 0.8.13 + Iris 1.10.7 + 90+ mods**  

### 纹理图集截断记录

单次游戏会话中，44 个纹理图集被追踪，**26 次超限截断**：
...
| Atlas 追踪 | 44 次 |
| Atlas 截断 | **26 次** |
| 格式降精度 | 0（1.21.11 无 16-bit 格式） |
| 深度降精度 | 0（1.21.11 仅 DEPTH32） |
| 阴影贴图截断 | 0（未超 1024 限制） |
| 预算告警 | 0 次 |
| 崩溃 | **0 次**（90+ mod 兼容） |
