...
public class MetricsEngine {
    ...
    public static void onFrame(float deltaMs) {
        ...
        if (elapsed >= intervalSec * 1000L) {
            ...
            currentFrameTimeMs = frameTimeCount > 0 ? frameTimeAccum / (float)frameTimeCount : 0;
    ...
    private static void pollVRAM() {
        try {
            ...
            long freeKB = GlVramProbe.INSTANCE.freeKB();
            if (totalKB <= 0 || freeKB < 0) return;
            lastVramTotalMB = totalKB / 1024;
            lastVramUsedMB = Math.max(0, (totalKB - freeKB) / 1024);
